...
import os
import tempfile
from apps.face_features.face_features import optical_face_features, haircut_face_features
...
class HaircutView(View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            results, success, img_base64 = haircut_face_features(image_path)
            os.unlink(temp_file.name)
            
            if success: return JsonResponse({'success': True, 'data': results, 'image': img_base64})
            
            return JsonResponse({'success': False, 'data': None, 'image': img_base64 if img_base64 else None })
        
        except Exception as e:
            print('Error: {}'.format(e))
            return JsonResponse({'success': False, 'data': None, 'image': None})